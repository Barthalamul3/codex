...
use codex_core::landlock::spawn_command_under_linux_sandbox_for_policies;
#[cfg(target_os = "macos")]
use codex_core::seatbelt::spawn_command_under_seatbelt_for_policies;
...
async fn load_debug_sandbox_config_with_codex_home(
    ...
    if config.uses_permission_profiles() {
        if full_auto {
            anyhow::bail!(
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        assert_eq!(
            config
                ...
                .resolve_access_with_cwd(docs.join("README.md").as_path(), config.cwd.as_path()),
        ...
        assert_eq!(
            config
                ...
                .resolve_access_with_cwd(
                    private.join("secret.txt").as_path(),
                    config.cwd.as_path(),
                ),
