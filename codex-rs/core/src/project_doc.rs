...
fn render_js_repl_instructions(config: &Config, model_info: Option<&ModelInfo>) -> Option<String> {
    if !config.features.enabled(Feature::JsRepl) {
        return None;
    }
    let can_request_original_image_detail = model_info.is_some_and(|model_info| {
        can_request_original_image_detail(config.features.get(), model_info)
    });
    ...
    if can_request_original_image_detail {
        section.push_str("- Request full-resolution image processing with `detail: \"original\"`. Use this when high-fidelity image perception or precise localization is needed, especially for CUA agents.\n");
    }
    if can_request_original_image_detail {
...
pub(crate) async fn get_user_instructions(
    ...
    if let Some(js_repl_section) = render_js_repl_instructions(config, model_info) {
        if !output.is_empty() {
            output.push_str("\n\n");
        }
...
mod tests {
    ...
    async fn no_doc_file_returns_none() {
        ...
        let res =
            get_user_instructions(&make_config(&tmp, 4096, None).await, None, None, None).await;
        assert!(
            res.is_none(),
    ...
    async fn doc_smaller_than_limit_is_returned() {
        ...
        let res = get_user_instructions(&make_config(&tmp, 4096, None).await, None, None, None)
            .await
            .expect("doc expected");
    ...
    async fn doc_larger_than_limit_is_truncated() {
        ...
        let res = get_user_instructions(&make_config(&tmp, LIMIT, None).await, None, None, None)
            .await
            .expect("doc expected");
    ...
    async fn zero_byte_limit_disables_docs() {
        ...
        let res = get_user_instructions(&make_config(&tmp, 0, None).await, None, None, None).await;
        assert!(
            res.is_none(),
    ...
    #[tokio::test]
    async fn js_repl_image_detail_original_adds_original_detail_guidance() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let mut cfg = make_config(&tmp, 4096, None).await;
        let mut features = cfg.features.get().clone();
        features
    ...
    async fn js_repl_image_detail_original_omits_guidance_without_model_support() {
        ...
        assert!(!res.contains("detail: \\\"original\\\""));
    }
    ...
    async fn apps_feature_does_not_emit_user_instructions_by_itself() {
        ...
        let res = get_user_instructions(&cfg, None, None, None).await;
        assert_eq!(res, None);
    }
