...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn view_image_tool_always_feature_still_rejects_unsupported_detail_values()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let rel_path = "assets/unsupported-detail-always.png";
    let abs_path = cwd.path().join(rel_path);
    if let Some(parent) = abs_path.parent() {
        std::fs::create_dir_all(parent)?;
    }
    ...
    let call_id = "view-image-unsupported-detail-always";
    let arguments = serde_json::json!({ "path": rel_path, "detail": "low" }).to_string();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn view_image_tool_always_feature_preserves_original_resolution_without_request()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let rel_path = "assets/original-example-always.png";
    let abs_path = cwd.path().join(rel_path);
    if let Some(parent) = abs_path.parent() {
        std::fs::create_dir_all(parent)?;
    }
