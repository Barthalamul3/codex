...
Output instructions:
- First determine a timestamp in `YYYY-MM-DD_HH-MM-SS` format using the current local session time.
- Write the full review to `/opt/ai/Documents/ClawdBrainVault/Projects/codex-upstream/.worktrees/per-turn-memory-architecture/runs/memory_os_reviews/memory_os_review_<TIMESTAMP>.md`.
- Do not append; create a new file for that timestamped run.
- After writing the file, respond in chat with only:
  `Saved review to /opt/ai/Documents/ClawdBrainVault/Projects/codex-upstream/.worktrees/per-turn-memory-architecture/runs/memory_os_reviews/memory_os_review_<TIMESTAMP>.md`
